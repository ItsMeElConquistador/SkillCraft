...
public class SCVersion implements Runnable {
	...
	private static SCVersion instance = new SCVersion();

	private static final String REMOTE_VERSION_XML_FILE = "https://raw.github.com/ItsMeElConquistador/SkillCraft/master/version.xml";
		...
		try {
			...
			if(remoteVersionProperty != null) {
				...
				if(remoteVersion != null) {
					if(!SCConfig.LAST_DISCOVERED_VERSION.equalsIgnoreCase(remoteVersion)) {
						SCConfig.set(SCConfig.CATEGORY_VERSION, "last_discovered_version", remoteVersion);
					}
	...
	private static String getVersionForCheck() {
		String[] versionTokens = SCReference.VERSION.split(" ");

		if(versionTokens.length >= 1)
			return versionTokens[0];
		else
			return SCReference.VERSION;
	}

	public static void logResult() {
		if(result == CURRENT || result == OUTDATED) {
			SCLog.log(Level.INFO, getResultMessage());
		} else {
			SCLog.log(Level.WARNING, getResultMessage());
		}
	}
	...
	public static String getResultMessage() {
		if(result == UNINITIALIZED) {
			...
			returnString = returnString.replace("@MOD_NAME@", SCReference.NAME);
			returnString = returnString.replace("@REMOTE_MOD_VERSION@", remoteVersion);
			returnString = returnString.replace("@MINECRAFT_VERSION@", Loader.instance().getMCVersionString());
	...
	public static String getResultMessageForClient() {
		String returnString = LanguageManager.getLocalization(OUTDATED_MESSAGE);
		returnString = returnString.replace("@MOD_NAME@", SCColors.TEXT_COLOUR_PREFIX_YELLOW + SCReference.NAME + SCColors.TEXT_COLOUR_PREFIX_WHITE);
		returnString = returnString.replace("@REMOTE_MOD_VERSION@", SCColors.TEXT_COLOUR_PREFIX_YELLOW + SCVersion.remoteVersion + SCColors.TEXT_COLOUR_PREFIX_WHITE);
		returnString = returnString.replace("@MINECRAFT_VERSION@", SCColors.TEXT_COLOUR_PREFIX_YELLOW + Loader.instance().getMCVersionString() + SCColors.TEXT_COLOUR_PREFIX_WHITE);
		returnString = returnString.replace("@MOD_UPDATE_LOCATION@", SCColors.TEXT_COLOUR_PREFIX_YELLOW + SCVersion.remoteUpdateLocation + SCColors.TEXT_COLOUR_PREFIX_WHITE);
		return returnString;
	}
	...
	public void run() {
		...
		SCLog.log(Level.INFO, LanguageManager.getLocalization(VERSION_CHECK_INIT_LOG_MESSAGE) + " " + REMOTE_VERSION_XML_FILE);

		try {
			while(count < SCReference.VERSION_CHECK_ATTEMPTS - 1 && (result == UNINITIALIZED || result == ERROR)) {
