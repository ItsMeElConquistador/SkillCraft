package elcon.mods.skillcraft.language;
...
import elcon.mods.skillcraft.SCUtil;
import elcon.mods.skillcraft.SkillCraft;
...
public class LanguageManager {
	...
	public static void load() {
		if(!loaded) {
			SCLog.info("[LanguageManager] Loading languages...");
			languageDirectory = new File(SkillCraft.minecraftDir, "/lang/agecraft/");
			languageDirectory.delete();
			languageDirectory.mkdirs();
			if(SkillCraft.modContainerSource.isDirectory()) {
				try {
					FileUtils.copyDirectory(new File(SkillCraft.modContainerSource, "/assets/agecraft/lang/"), languageDirectory);
				} catch(Exception e) {
					e.printStackTrace();
				}
			} else {
				try {
					JarFile jar = new JarFile(SkillCraft.modContainerSource);
	...
	private static void searchLanguageDirectory(File dir) {
		File[] files = dir.listFiles(new FileFilter() {
			...
			public boolean accept(File file) {
				return file.isDirectory() || SCUtil.getFileExtension(file).equals("lang");
			}
		});
	...
	public static void loadLanguageFile(File file) {
		...
		try {
			SCLog.info("[LanguageManager] Loading language file: " + dirName + "/" + fileName);
			FileInputStream inputStream = new FileInputStream(file);

			String language = fileName.replace(SCUtil.getFileExtension(fileName), "").replace(".", "");
			...
			SCLog.severe("[LanguageManger] Error while reading language file (" + fileName + "): ");
			e.printStackTrace();
		}
	}
